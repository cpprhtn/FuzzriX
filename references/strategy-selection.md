...
|---|---|---|
| **value_profile** | `-use_value_profile=1` | **On by default** (libFuzzer ships it *off*; FuzzriX flips it on). Tracks interesting compared values (e.g. `memcmp`/magic-number checks) to get past hard equality gates. Not free: libFuzzer documents up to ~2x slowdown and the corpus can grow several times — worth it for parsers/decoders, drop it if exec/s tanks on a hot target. Needs `-fsanitize-coverage=trace-cmp` (on by default as part of `-fsanitize=fuzzer`). |
| **recommended_dict** | `-dict=<file>` | **On whenever a dict exists or is worth generating.** Token dictionary (magic bytes, keywords) dramatically helps structured formats. Look for `<binary>.dict` beside the target; else mine string/byte literals from the target into one. Validate the file exists before passing the flag — stock libFuzzer errors on a missing dict. |
