...
| **recommended_dict** | `-dict=<file>` | **A *huge* win behind a string/magic gate; small but positive otherwise** (so always worth applying). Magic-byte/keyword tokens get past `memcmp(data,"MAGIC",..)`-style gates the mutator can't guess; on punctuation grammars (JSON) the lift is smaller — but never tune this off one run (single trials are noise — see Discipline). Use `<binary>.dict` if it ships; else **mine one with [`scripts/mine_dict.py`](../scripts/mine_dict.py)** (`python3 scripts/mine_dict.py <repo> -o fuzz.dict`) — it pulls string literals + magic byte-arrays from source and escapes them. Validated: on `stb_image` a mined dict found a decoder crash within 20 s that no-dict/value-profile runs missed. Confirm the file exists before passing `-dict` (libFuzzer errors on a missing one). |
